...
const BookingManagement: React.FC = () => {
  ...
  useEffect(() => {
    // Check if any checkbox is checked
    const anyChecked = checkboxStates.some((checked) => checked);
    setAreAllCheckboxesSelected(
      anyChecked && checkboxStates.every((checked) => checked)
    );
  }, [checkboxStates]);
  ...
  return (
    <>
      <h2 className="text-center font-bold text-2xl">Bookings</h2>
      ...
      <div className="bg-white shadow-md rounded-md px-6 pb-6 pt-2 flex h-screen">
        <div className="w-3/4 mb-4 shadow-2xl shadow-slate-400 border-2">
          <div className="flex w-full bg-slate-300">
            <div className="h-fit flex w-full text-lg font-bold text-center">
              ...
              <div className="w-3/12 bg-green-300 font-bold mt-2 mr-1">
                Name
              </div>
              <div className="w-3/12 bg-green-300 font-bold mt-2 mr-1">
                Email
              </div>
              <div className="w-2/12 bg-green-300 font-bold mt-2 mr-1">
                Contact
              </div>
              <div className="w-1/12 bg-pink-300 mt-2 mr-1">Date</div>
              <div className="w-1/12 bg-pink-300 mt-2 mr-1">View</div>
            ...
            {admins.map((admin, index) => (
              <React.Fragment key={index}>
                <div className="flex w-full bg-slate-300 text-center ">
                  <div className="w-1/12 bg-blue-300 mt-2 ml-2 mr-1">
                    {admin.index}
                  </div>
                  <div className="w-3/12 bg-green-300 font-bold mt-2 mr-1">
                    {admin.name}
                  ...
                  <div className="w-1/12 bg-pink-300 mt-2 mr-1 cursor-pointer flex justify-center items-center">
                    {admin.date}
                  </div>
                  <div
                    className="w-1/12 bg-pink-300 mt-2 mr-1 cursor-pointer flex justify-center items-center"
                    onClick={() => viewUser(index)}
                  >
                    {<FaEye size={20} />}
                  </div>
                  <div
                    className="w-1/12 bg-cyan-300 mt-2 mr-2 cursor-pointer flex justify-center items-center"
        ...
        <div className="w-1/4 mb-4 border-2 shadow-2xl shadow-slate-400 ml-4 flex flex-col">
          ...
          <button
            ...
            } text-white py-2 px-4 m-2 rounded-lg shadow-md`}
          >
