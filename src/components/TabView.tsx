...
const OverviewTab = ({price,phone,address,willingToTravel}: 
    {price:string | undefined, phone:string|undefined, address: string|undefined, willingToTravel: boolean|undefined}) => {
...
const ContactTab = ({ phone, address }: 
    { phone: string | undefined, address: string | undefined }) => {
...
const PackageTab = ({ packages }: 
    { packages?: { name: string; price: string; days: string; minAdvance: string} | undefined}) => {
    if (!packages) {
        ...
        <div className="flex flex-col items-center">
            ...
            <div className="grid grid-cols-2 gap-8">
                {/* {packages?.map((pkg, index) => (
                    ...
                    > */}
                        <h3 className="font-semibold text-lg font-roboto">Name: {packages.name}</h3>
                        <p className="font-semibold text-lg font-roboto">Price: {packages.price}</p>
                        <p className="font-semibold text-lg font-roboto">Duration: {packages.days}</p>
                        <p className="font-semibold text-lg font-roboto">Minimum Advance: {packages.minAdvance}</p>
                    {/* </div> */}
                {/* ))} */}
            </div>
        </div>
...
const RatingsTab = ({ratings}: {ratings?:{ stars: number; review: string; }[]|undefined}) => {    
